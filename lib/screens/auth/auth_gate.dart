...
class _AuthGateState extends State<AuthGate> {
  ...
  Future<void> _registrarLoginEVersao(String uid) async {
    if (_lastUidProcessed == uid) return; // Já processou este usuário nesta sessão
  ...
  Widget build(BuildContext context) {
    return StreamBuilder<User?>(
      ...
      builder: (context, authSnapshot) {
        ...
        WidgetsBinding.instance.addPostFrameCallback((_) => _registrarLoginEVersao(user.uid));
          ...
          builder: (context, userDocSnapshot) {
            ...
            return StreamBuilder<DocumentSnapshot>(
              ...
              builder: (context, storeSnapshot) {
                ...
                if (storeData == null) return const SubscriptionScreen(storeId: '');
                ...
                if (trialEndDate != null) {
                  try {
                    isTrialValid = DateTime.now().isBefore(DateTime.parse(trialEndDate));
                  } catch (_) {}
                }

                // 🚪 Regra de Acesso: Ativo ou dentro do Trial (mesmo que Pending)
                if (status == 'active' || isTrialValid) {
