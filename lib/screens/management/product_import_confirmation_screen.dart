...
class ProductImportConfirmationScreen extends StatefulWidget {
  ...
  @override
  State<ProductImportConfirmationScreen> createState() =>
      _ProductImportConfirmationScreenState();
}

class _ProductImportConfirmationScreenState
    extends State<ProductImportConfirmationScreen> {
  final ProductImportService _service = ProductImportService();
  ...
  String _importErrorMessage(Object error) {
    const fallback = 'Não foi possível concluir a importação. Tente novamente.';
  ...
  Future<void> _importProducts() async {
    ...
    if (analysis == null || analysis.newProducts.isEmpty) {
      return;
    }
    ...
    try {
      ...
      Navigator.of(context).popUntil(
        (route) => route.isFirst || route.settings.name == '/manage-products',
      );
  ...
  List<String> _newCategoryLabels(ProductImportAnalysis analysis) {
    ...
    for (final resolution in analysis.categoryResolutions.values) {
      ...
      final categoryId = resolution.categoryId?.trim();
      ...
      final subcategoryId = resolution.subcategoryId?.trim();

      if (subcategoryId == null || subcategoryId.isEmpty) {
        final normalizedSubcategory = _normalizeCategoryName(subcategoryName);

        labelsByKey.putIfAbsent(
          'child:$normalizedCategory:$normalizedSubcategory',
          () => 'Subcategoria: $categoryName › $subcategoryName',
        );
      }
    }
  ...
  String _productCategorySuffix(ProductImportItem product) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Confirmar Importação')),
        ...
        child: ConstrainedBox(
          ...
          child: ListView(
            ...
            children: [
                ...
                children: [
                  Expanded(
                    child: _summary(
                      ...
                      value: analysis.newProducts.length,
                      label: 'Novos',
                      color: Colors.green,
                    ),
                  ),
                  ...
                  Expanded(
                    child: _summary(
                      ...
                      value: analysis.duplicateProducts.length,
                      label: 'Duplicados',
                      color: Colors.orange,
                    ),
                  ),
                  ...
                  Expanded(
                    child: _summary(
                      ...
                      value: _newCategoryLabels(analysis).length,
                      label: 'Categorias',
                      color: Colors.blue,
                    ),
                  ),
              ...
              // CATEGORIAS NOVAS
              // ==============================================================
              if (_newCategoryLabels(analysis).isNotEmpty) ...[
                ...
                Wrap(
                  spacing: 8,
                  runSpacing: 8,
                  children: _newCategoryLabels(analysis)
                      .map(
                        (category) => Chip(
              ...
              // DUPLICADOS
              // ==============================================================
              if (analysis.duplicateProducts.isNotEmpty) ...[
                const SizedBox(height: 28),
                ...
                ...analysis.duplicateProducts.map(
                  (product) => Card(
                    child: ListTile(
                      leading: const Icon(
                        Icons.content_copy,
                      ...
                      subtitle: Text('Linha ${product.sourceRow} da planilha'),
                    ),
                  ),
                ),
              ],
                ...
                (product) => Card(
                  child: ListTile(
                    ...
                    subtitle: Text(
                      ...
                      '${_productCategorySuffix(product)}',
                    ),
                  ),
                ),
              ),
              ...
              SizedBox(
                ...
                child: ElevatedButton.icon(
                  onPressed: _isImporting || analysis.newProducts.isEmpty
                      ? null
                      : _importProducts,
