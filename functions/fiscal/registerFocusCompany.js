...
//
// SEGURANÇA:
// - Requer Firebase Auth.
// - Somente proprietário da loja.
// - Somente plano Business ativo.
//
// INTEGRAÇÕES:
// - Firebase Firestore
// - Focus NFe API
//
// CREDENCIAIS:
// - FOCUS_NFE_MASTER_TOKEN no backend.
// - Nunca expor token ao Flutter.
//
// FLUXO:
// Flutter -> Cloud Function -> Focus NFe -> Firestore
//
// MODO DE TESTE:
// dryRun = true não efetiva o cadastro.
//
// ============================================================================
...
// 2. Garantir que ele seja o proprietário da loja.
// 3. Garantir que a loja possua plano Business ativo.
// 4. Validar os dados fiscais salvos no Firestore.
// 5. Consultar se a empresa já existe na Focus.
// 6. Validar o cadastro usando dry_run=1.
// 7. Posteriormente permitir o cadastro real.
// 8. Salvar no Firestore somente o estado da integração.
//
// IMPORTANTE:
// - O token da Focus nunca vem do Flutter.
// - O token fica somente no backend (.env).
// - Esta função NÃO envia certificado digital ainda.
// ============================================================================

const registerFocusCompany = onCall(async (request) => {
  // ==========================================================================
  // 1. AUTENTICAÇÃO
  // ==========================================================================
  ...
  // SOMENTE o ownerId da loja pode executar a operação.
  //
  // Funcionários e gerentes ficam bloqueados no backend,
  // mesmo que tentem chamar a Cloud Function diretamente.
  // ==========================================================================
  ...
  // Número
  //
  // A API de Empresas documenta este campo como inteiro.
  // Nesta primeira implementação exigiremos um número.
  // --------------------------------------------------------------------------
  ...
  //
  // A documentação da Focus define inscrição_estadual como inteiro.
  //
  // Porém não vamos enviar valor 0 artificialmente.
  // Se não houver números preenchidos, simplesmente não adicionamos
  // o campo ao payload.
  // ==========================================================================
  ...
  //
  // Nunca mandar esse token para o Flutter ou Firestore.
  // ==========================================================================
  ...
  //
  // A API de Empresas funciona no servidor de produção.
  //
  // Para testar a criação utilizamos:
  //
  // dry_run=1
  //
  // e NÃO a URL de homologação.
  // ==========================================================================
  ...
  //
  // Começamos apenas com os dados necessários à nossa integração NFC-e.
  // ==========================================================================
  ...
  const companyPayload = {
    ...
    //
    // Nesta fase o Store Connect Business trabalhará com NFC-e.
    // ------------------------------------------------------------------------
  ...
  //
  // Somente adicionamos se realmente houver valor numérico.
  // ==========================================================================
  ...
  // 19. EMAIL
  //
  // Se futuramente tivermos email comercial da loja,
  // podemos usar ele.
  //
  // Por enquanto só enviamos caso já exista no documento.
  // ==========================================================================
  ...
  try {
    ...
    if (dryRun === true) {
      ...
      return {
        ...
        // Não precisamos mandar todos os dados da Focus
        // ao aplicativo.
        focusStatus:
          response.status,
      };
    ...
    //
    // Guardamos detalhes suficientes para diagnosticar.
    //
    ...
    // ------------------------------------------------------------------------
    // Guarda apenas o estado do erro no perfil da integração.
    //
    // Não alteramos os demais campos do perfilFiscal.
    // ------------------------------------------------------------------------
    ...
    //
    // Isso será muito útil durante o desenvolvimento para mostrar
    // exatamente qual campo a Focus recusou.
    // ------------------------------------------------------------------------
